@echo off
:: Launches Taboryx AI with Windows Administrator privileges (required for Wi-Fi management)
...
if errorlevel 1 (
    echo Starting Taboryx mobile API server...
    start "Taboryx API" /MIN cmd /c "cd /d \"%CD%\" && \"%PYTHON_EXE%\" -m src.api.server --host 0.0.0.0 --port 8000"
...
echo Running as Administrator. Launching Taboryx AI...
